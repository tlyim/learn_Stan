...
#knitr::purl("ssm_Stan0.4.4dev.Rmd", output = "ssm_Stan0.4.4dev.R", documentation = 2)
# .libPaths()
...
#===========================================
J = 20#15#10#5#42#40#45#20 #200     # number of firms
...
g = c(0.03, 0.23) #[before rescaled X2] g = c(0.03, 2.3)   # g = c(0.5, 1)
#g = c(0.5, 0.5, 1) 
K = length(g)
...
simu_fit <- stan(#model_code = simu_SSM@model_code,
                 file = "simu_SSM0.4.4dev.stan",  # Stan program
...
file = "SSM0.4.4dev.stan",  # Stan program
...
print(fit2, c(
              "mu_u1", 
              "mu_alpha", "beta", #"sd_base", #"mu_base", #"theta", #"rho", 
...
#mcmc_pairs(posterior, np = np, pars = c("sd_base", "mu_u1", "sd_y", "w_mu[1]", "w_mu[2]"))
mcmc_pairs(posterior, np = np, pars = c("p_mu[1]", "p_mu[2]", "p0[1]", "w_mu[1]", "w_mu[2]"))
...
# mcmc_pairs(posterior, np = np, pars = c("sd_base", "ab_mu[1]", "ab_mu[2]", "mu_u1", "sd_y"))
# mcmc_pairs(posterior, np = np, pars = c("sd_base", "w_mu[1]", "w_mu[2]"))
# mcmc_pairs(posterior, np = np, pars = c("sd_base", "g_mu[1]", "g_mu[2]"))
# mcmc_pairs(posterior, np = np, pars = c("sd_base", "p_mu[1]", "p_mu[2]", "p0[1]"))
# mcmc_pairs(posterior, np = np, pars = c("sd_base", "d[1]", "d[2]", "d[3]"))

mcmc_pairs(posterior, np = np, pars = c("ab_mu[1]", "ab_mu[2]", "w_mu[1]", "w_mu[2]"))
mcmc_pairs(posterior, np = np, pars = c("ab_mu[1]", "ab_mu[2]", "g_mu[1]", "g_mu[2]"))
mcmc_pairs(posterior, np = np, pars = c("ab_mu[1]", "ab_mu[2]", "p_mu[1]", "p_mu[2]", "p0[1]"))
...
#mcmc_pairs(posterior, np = np, pars = c("sd_base", "mu_u1", "sd_y", "g_mu[1]", "g_mu[2]"))
mcmc_pairs(posterior, np = np, pars = c("p_mu[1]", "p_mu[2]", "p0[1]", "g_mu[1]", "g_mu[2]"))
mcmc_pairs(posterior, np = np, pars = c("ab_mu[1]", "ab_mu[2]", "g_mu[1]", "g_mu[2]"))
...
# s = c(0.4, 0.7, 0.5) 
  # p = c(0.7, 0.3, 0.13)  #[rho=1] p = c(0.37, 3.0, 0.13)   # soft-clipping [rho=5]; 
  # g = c(0.03, 0.23)  #[rho=1] g = c(0.5, 1)    
  # w = c(0.1, 0.57)  #[rho=1] w = c(4.3, 0.6)
  # d = c(0.05, 0.7, 0.25) 
...
# theta = 0.1; rho = 5,  
  ...
  # sd_y = 0.15, sd_season = 0.1, mu_season = c(-0.12, -0.06, 0.15); 
# s = c(0.4, 0.7, 0.5) 
  # p = c(0.7, 3.0, 0.13)  #[rho=1] p = c(0.37, 3.0, 0.13)   # soft-clipping [rho=5]; 
  # g = c(0.03, 0.23)  #[rho=1] g = c(0.5, 1)    
  # w = c(0.1, 0.57)  #[rho=1] w = c(4.3, 0.6)
  # d = c(0.05, 0.7, 0.25) 
...
# mu_base = -1.6, sd_base = 0.05;  
  # sd_y = 0.15, sd_season = 0.1, mu_season = c(-0.12, -0.06, 0.15); 
# s = c(0.4, 0.7, 0.5) 
# p = c(0.37, 3.0, 0.13)   # soft-clipping [rho=5]; 
