...
generated quantities {
  ...
  for (j in 1:J) {
    for (k in 1:K) {
      gamma[j,k] = normal_rng(c[k], sd_gamma);
    }
    m[j,1] = normal_rng(X[j,1]*gamma[j], sd_m);
    y[j,1] = normal_rng(a/(1-b), sd_y);   // y should be nonnegative for Rev and nonpositive for Costs
// Perhaps best to model y as always positive, with IsCost = 1, 0 to indicate Cost or Rev item 
    for (n in 2:N) {
      ...
      y[j,n] = a + normal_rng(b*y[j,n-1], sd_y);
    }
  }
