...
#knitr::purl("ssm_Stan0.1.3test.Rmd", output = "ssm_Stan0.1.3test.R", documentation = 2)
...
#===========================================
J = 100#60#32#45#20 #200     # number of firms
N = 4*20#12#5#15   #number of (quarterly) observations of a firm
...
mu_alpha = 0.2  # intercept parameter of the AR(1) processs of the underlying unbiased figure
#mu_alpha = 0.04  # intercept parameter of the AR(1) processs of the underlying unbiased figure
mu_alpha01 = 0.5*(1 + mu_alpha);
beta = 0.85  # slope parameter of the AR(1) processs of the underlying unbiased figure
...
rho_raw = log(rho)/1 #1
alpha_ratio = mu_alpha/rho #0.1
alpha_ratio_raw = log(alpha_ratio) 
...
simu_fit <- stan(#model_code = simu_SSM@model_code,
                 file = "simu_SSM0.1.3test.stan",  # Stan program
...
initf2 <- function(chain_id = 1) {
  list(mu_alpha01 = mu_alpha01, mu_alpha = mu_alpha, beta = beta,  
       p = p, g = g, w = w, d = d, mu_season = mu_season, sd_season = sd_season,
       mu_u1 = y_init, #u = array(y_LT, dim = c(J,N)),
       rho_ST = rho_ST, rho = rho, rho_raw = rho_raw, alpha_ratio_raw = alpha_ratio_raw, alpha_ratio = alpha_ratio,
       sd_pi = sd_pi, sd_y = sd_y, sd_base = sd_base, sd_gamma = sd_gamma, sd_omega = sd_omega)
  }
...
file = "SSM0.1.3test.stan",  # Stan program
...
fit2 <- fit1
save(fit2 , file="/scratch/city/fit2.rda")
# beepr::beep()
...
print(fit2, c("mu_u1", "mu_alpha", "beta", #"rho_raw", #"rho", 
              "p", #"p_raw",#"sd_pi",  #"rho_ST", "alpha_ratio_raw", "alpha_ratio", 
#              "g", "w", "d", #"mu_season", "sd_season", " 
              "sd_y", "sd_season", "mu_season" # "season_raw" 
              ), #"sd_base"),#, "sd_gamma", "sd_omega"),
...
pairs_stan(1, fit2, c("mu_u1", "mu_alpha", "beta", "sd_y", "sd_season", "mu_season"))#"season_raw"))#, 
pairs_stan(1, fit2, c("mu_alpha", "beta", "p"))#"season_raw"))#, "sd_pi", "p"
...
#      xlab="Iteration")#, ylab="log(tau)", ylim=c(-6, 4))
#plot(params.df$iter, params.df$p[1], pch=16, cex=0.8, #col=c_dark, 
#     xlab="Iteration")#, ylab="log(alpha_ratio)")#, ylim=c(-6, 4))
#plot(params.df$iter, params.df$rho_raw, pch=16, cex=0.8, #col=c_dark, 
...
#====================================================
    # mu_u1 = y_init = 4; mu_alpha = 0.2; beta = 0.85;  
# rho_raw = 0.693;
# rho = 2, rho_ST = 0.1,  
# alpha_ratio_raw = -2.3, alpha_ratio = 0.1, 
# [rho=5] p = c(-3.0, 0.13, -0.37); [old] p = c(-4.25, -1, 3.5); 
    # g = c(0.5, -0.8, 2.5)
    # w = c(-3.5, 7, 2)
    # d = c(0.05, 0.7, 0.25) 
