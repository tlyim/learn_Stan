...
// Need to specify parameters{} and model{} this time to use MCMC to estimate the latent GP's parameters
parameters {
...
// The model for trying to use MCMC to match the observed y
model {
  f_tilde ~ normal(0, 1);
...
generated quantities {
  ...
  for (n in 1:N_predict)
    y_predict[n] = poisson_log_rng(log_f_predict[n]);  // Should this be just f_predict[n] ?
}
