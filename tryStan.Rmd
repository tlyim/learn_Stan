...
# Simulate GP data
```{stan output.var=simu_GP, eval=T}
data {
  int<lower=1> N;
  real x[N];
...
```{stan output.var=misreport, eval=T}
...
// b ~ cauchy(0, 2.5); // common prior for each b[K]
  ...
  rho ~ inv_gamma(5,30);    // (5,5) (8.91924, 34.5805);
  alpha ~ normal(0, 1);
  sigma ~ normal(0, 1);

/* Note: Note sure why a nugget or jitter of 10^−10 is not added, with its place apparently replcated by 'square(sigma)'

so the marginal covariance matrix before computing its Cholesky decomposition in order to stabilize the numerical calculations. Often the nugget is taking to be square root of the floating point precision, which would be 10−8 for double precision calculations.
 https://betanalpha.github.io/assets/case_studies/gp_part1/part1.html
*/
...
set.seed(123)  # set the seed for random generation to ensure replicability
...
simu_fit <- stan(model_code = simu_GP@model_code, data=sim_GP_data, iter=1,
            chains=1, seed=321, algorithm="Fixed_param")
