...
//    alpha[j,1] = mu_alpha + sd_y * err_alpha[j,1] - RealLT[j,1];   // y should be nonnegative for Rev and nonpositive for Costs
    ...
    y[j,1] = y_init + RealST[j,1] + sd_y * err_alpha[j,1];   // y should be nonnegative for Rev and nonpositive for Costs
    for (n in 2:N) {
      err_alpha[j,n] = normal_rng(0, 1);
 //     alpha[j,n] = alpha[j,n-1] + sd_y * err_alpha[j,n] - RealLT[j,n];
      alpha[j,n] = alpha[j,n-1] - RealLT[j,n];
      y[j,n] = alpha[j,n-1] + beta*y[j,n-1] - delta*RealST[j,n-1] + RealST[j,n] + sd_y * err_alpha[j,n]; //- delta*RealST[j,n-1];
      }
