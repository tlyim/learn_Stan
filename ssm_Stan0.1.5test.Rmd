...
#knitr::purl("ssm_Stan0.1.5test.Rmd", output = "ssm_Stan0.1.5test.R", documentation = 2)
...
#===========================================
J = 40#45#20 #200     # number of firms
N = 4*15#5#15   #number of (quarterly) observations of a firm
...
get_ratioNED <- function(ratioNED_req, J, N) {
  C = pmax(ratioNED_req, rbinom(J, 20, ratioNED_req)/20) 
  EE = rep(NULL, J)
  for (n in 1:N)  { 
    A = rbinom(J, 1, 0.5)/20
    B = -rbinom(J, 1, 0.5)/20
...
simu_fit <- stan(#model_code = simu_SSM@model_code,
                 file = "simu_SSM0.1.5test.stan",  # Stan program
...
initf2 <- function(chain_id = 1) {
  list(mu_alpha = mu_alpha, beta = beta, #shape = shape, eta = eta, #sigma = sigma, mu_alpha01 = mu_alpha01, 
       ...
       rho = rho, sd_y = sd_y, sd_base = sd_base, sd_gamma = sd_gamma, sd_omega = sd_omega)
  }
...
file = "SSM0.1.5test.stan",  # Stan program
...
print(fit2, c("mu_u1", "mu_alpha", "beta", #"s", #"eta", #"rho_raw", #"rho", 
              #"p",  
              "g", 
              "w", "d", #"mu_season", "sd_season", " 
...
mcmc_pairs(posterior, np = np, pars = c("mu_alpha", "beta", "sd_y", "sd_season"))#,"w", "d")), "mu_season"
mcmc_pairs(posterior, np = np, pars = c("mu_alpha", "sd_y", "w[1]", "w[2]", "w[3]"))#,"w", "d"))
mcmc_pairs(posterior, np = np, pars = c("beta", "sd_season", "w[1]", "w[2]", "w[3]"))#,"w", "d"))

mcmc_pairs(posterior, np = np, pars = c("mu_alpha","w[1]", "g[1]", "g[2]", "g[3]"))#,"w", "d"))
mcmc_pairs(posterior, np = np, pars = c("beta","w[2]", "g[1]", "g[2]", "g[3]"))#,"w", "d"))
mcmc_pairs(posterior, np = np, pars = c("sd_y","w[3]", "g[1]", "g[2]", "g[3]"))#,"w", "d"))

mcmc_pairs(posterior, np = np, pars = c("beta","w[1]", "d[1]", "d[2]", "d[3]"))#,"w", "d"))
mcmc_pairs(posterior, np = np, pars = c("sd_y","w[2]", "d[1]", "d[2]", "d[3]"))#,"w", "d"))
mcmc_pairs(posterior, np = np, pars = c("sd_season","w[3]", "d[1]", "d[2]", "d[3]"))#,"w", "d"))
...
# w = c(-3.7, 4.3, 0.6)         #[old] w = c(-3.5, 7, 2)
    # d = c(0.05, 0.7, 0.25) 
