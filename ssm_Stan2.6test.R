...
#knitr::purl("ssm_Stan2.6test.Rmd", output = "ssm_Stan2.6test.R", documentation = 2)
...
simu_fit <- stan(#model_code = simu_SSM@model_code,
                 file = "simu_SSM2.6test.stan",  # Stan program
...
initf2 <- function(chain_id = 1) {
  list(mu_alpha01 = mu_alpha01, beta = beta, # alpha = alpha, 
       ...
       rho_ST = rho_ST, rho_LT = rho_LT, sd_pi = sd_pi, #sd_Rm = sd_Rm, #delta = delta,  
...
file = "SSM2.6test.stan",  # Stan program
...
print(fit2, c("mu_alpha", "beta", "mu_u1", "rho_ST", #"rho_LT", #"mu_u1", #"delta", 
              "p", "g", "w", "d", "mu_season", "sd_season",
              "sd_pi", "sd_y", "sd_gamma", "sd_omega", "sd_base"),
...
pairs_stan(2, fit2, c("mu_alpha", "beta", "sd_y", "sd_gamma", "g"))
pairs_stan(1, fit2, c("sd_season", "mu_season"))
pairs_stan(2, fit2, c("sd_season", "mu_season"))
pairs_stan(1, fit2, c("mu_u1", "rho_ST", "sd_pi", "p"))#"delta", "rho_LT", 
pairs_stan(2, fit2, c("mu_u1", "rho_ST", "sd_pi", "p"))#"delta", "mu_u1", "rho_LT", 
pairs_stan(1, fit2, c("sd_base", "sd_omega", "w", "d"))
pairs_stan(2, fit2, c("sd_base", "sd_omega", "w", "d"))
...
#====================================================
    # mu_alpha = 0.04; mu_alpha01 = 0.52 
    # beta = 0.6  
# mu_u1 = 0.3, rho_ST = 0.1, rho_LT = 0.02,   #delta = 0.65,    
    # p = c(-4, -1, 3.5)
    # g = c(0.5, -0.8, 2.5)
    # w = c(-3.5, 7, 2)
    # d = c(0.05, 0.7, 0.25) 
# mu_season = c(-0.03, -0.06, 0.05)
# sd_season = 0.02, sd_y = 0.08   
# sd_pi = 0.05, sd_gamma = 0.05, sd_omega = 0.1, sd_base = 0.1  
