...
transformed data {
  ...
  int<lower=0> K_cor = 2; // number of correlated coefficents in p for covariate matrix Z
...
//    Real[j] = ( log1p_exp(rho*phi[j]) - log1p_exp(rho*(phi[j]-1)) )/rho;
    Real[j] = //exp(p[1])*
              ( log1p_exp(rho*phi[j]) - log1p_exp(rho*(phi[j]-1)) )/rho;
//===============================================================================
...
//    w ~ normal(0, 1);//student_t(4, 0, 1);//student_t(3, 0, 5);//normal(0, 5);//10);//0); //0.2); //1);  // w_init
        //w0 ~ normal(0, 1);//2);
        w_mu ~ student_t(3,0,1);//4, 0, 1);//normal(0, 1);
        w_sd ~ normal(0, 0.2);//0.2);//2);
        w_L ~ lkj_corr_cholesky(2);//2);
        w_err[1] ~ normal(0, 0.2);//0.2); // implies:  w_raw ~ multi_normal(w_mu, quad_form_diag(w_L * w_L', w_sd));
        w_err[2] ~ normal(0, 0.2);//0.2); // implies:  w_raw ~ multi_normal(w_mu, quad_form_diag(w_L * w_L', w_sd));
//===============================================================================
