...
#knitr::purl("ssm_Stan0.1.4test.Rmd", output = "ssm_Stan0.1.4test.R", documentation = 2)
...
simu_fit <- stan(#model_code = simu_SSM@model_code,
                 file = "simu_SSM0.1.4test.stan",  # Stan program
...
sigma.vec <- extract(simu_fit)$sigma[1,] %>%
 round(3) %>%
         t() %>%
         split(col(.)) %>%
          unlist() %>% 
          as.vector() 
cat("\n\nmean(sigma): ", mean(sigma.vec), "\n\n")
sigma.vec
...
file = "SSM0.1.4test.stan",  # Stan program
...
pairs_stan(1, fit2, c("mu_alpha", "beta", "s", "p"))#"season_raw"))#, "sd_pi", "eta", 
