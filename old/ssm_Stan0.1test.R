...
#knitr::purl("ssm_Stan0.1test.Rmd", output = "ssm_Stan0.1test.R", documentation = 2)
...
#===========================================
J = 150#45#20 #200     # number of firms
N = 4*20#5#15   #number of (quarterly) observations of a firm
...
rho_LT_raw = rho_LT - 1 #1
alpha_ratio = mu_alpha/rho_LT #0.1
...
simu_fit <- stan(#model_code = simu_SSM@model_code,
                 file = "simu_SSM0.1test.stan",  # Stan program
...
initf2 <- function(chain_id = 1) {
  list(mu_alpha01 = mu_alpha01, mu_alpha = mu_alpha, beta = beta,  
       ...
       rho_ST = rho_ST, rho_LT = rho_LT, sd_pi = sd_pi, rho_LT_raw = rho_LT_raw, alpha_ratio = alpha_ratio,     
...
file = "SSM0.1test.stan",  # Stan program
...
print(fit2, c("mu_u1", "mu_alpha", "beta", "rho_LT", "sd_pi", "p",# #"rho_ST", 
...
pairs_stan(1, fit2, c("mu_u1", "alpha_ratio", "beta", "sd_y", "sd_season", "mu_season"))#"season_raw"))#, 
pairs_stan(1, fit2, c("alpha_ratio", "beta", "rho_LT_raw", "p"))#"season_raw"))#, 
...
plot(params.df$iter, params.df$sd_pi, pch=16, cex=0.8, #col=c_dark, 
      xlab="Iteration")#, ylab="log(tau)", ylim=c(-6, 4))
plot(params.df$iter, params.df$alpha_ratio, pch=16, cex=0.8, #col=c_dark, 
     xlab="Iteration")#, ylim=c(0, 0.3))#, ylab="log(tau)", ylim=c(-6, 4))
