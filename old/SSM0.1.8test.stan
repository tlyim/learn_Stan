...
//Not working<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
//Reparametrize w; o/w the elements are correlated
//w[H] = exp(w_raw[H]);
//  for (h in 1:(H-1)) {
//    w[h] = exp(w_raw[h] + w_raw[H]); //exp(log(w_raw[h]));
//    }
//Not working<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
alpha[j,1] = mu_alpha;
for (n in 2:N) {
  alpha[j,n] = alpha[j,n-1] + (-0.1)*Real[j,n-1];  
  }
//===============================================================================
...
//mu_base[j] ~ normal(0, 1);
                    ...
                    //err_alpha[j,1] ~ normal(0, sd_alpha);
                    //err_alpha[j,2:N] ~ normal( (-0.1)*Real[j,1:(N-1)], sd_alpha );  // 
                    //^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Disabled Real[] 
