...
real<lower=0> alpha_ratio; // intercept coefficient (drift) of the AR(1) process of the unbiased figure y[n]
...
real<lower=0> rho_LT_raw; // coeff. of the LT impact of real EM
}
...
//vector[N] err[J];  // error term in p
  vector[Q] season_q[J];
  vector[N] season_n[J];
real rho_LT; // coeff. of the LT impact of real EM
vector[N] realLT[J]; // LT impact of real EM on alpha[j,n]
real mu_alpha; // intercept coefficient (drift) of the AR(1) process of the unbiased figure y[n]

rho_LT = 1 + rho_LT_raw;
mu_alpha = rho_LT * alpha_ratio;
...
//    RealLT[j] = log( log1p_exp((-rho_LT)*zeta[j]) - log1p_exp((-rho_LT)*(zeta[j]+1)) ) - log(rho_LT);
//    RealLT[j] = ( log1p_exp((-rho_LT)*zeta[j]) - log1p_exp((-rho_LT)*(zeta[j]+1)) )/rho_LT;
    realLT[j] = ( log1p_exp((-rho_LT)*zeta[j]) - log1p_exp((-rho_LT)*(zeta[j]+1)) );
//===============================================================================
...
sd_pi ~ normal(0, 1); 
rho_LT_raw ~ normal(0, 10);//1); // Cauchy(0,5), student_t(3, 0, 1); 
alpha_ratio ~ normal(0, 10); 
