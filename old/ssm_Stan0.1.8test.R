...
#knitr::purl("ssm_Stan0.1.8test.Rmd", output = "ssm_Stan0.1.8test.R", documentation = 2)
...
N = 4*25#15#5#15   #number of (quarterly) observations of a firm
...
simu_fit <- stan(#model_code = simu_SSM@model_code,
                 file = "simu_SSM0.1.8test.stan",  # Stan program
...
file = "SSM0.1.8test.stan",  # Stan program
...
print(fit2, c("mu_u1", "mu_alpha", "beta", "mu_base", "sd_base", 
...
mcmc_pairs(posterior, np = np, pars = c("mu_alpha", "beta", "mu_base", "sd_base", "sd_y", "sd_season"))#,"w", "d")), "mu_season"
#mcmc_pairs(posterior, np = np, pars = c("mu_base", "sd_base", "p_raw[1]", "p_raw[2]", "p_raw[3]"))#,"w", "d"))
mcmc_pairs(posterior, np = np, pars = c("mu_base", "sd_base", "p[1]", "p[2]", "p[3]"))#,"w", "d"))
mcmc_pairs(posterior, np = np, pars = c("mu_base", "sd_base", "g[1]", "g[2]", "g[3]"))#,"w", "d"))
mcmc_pairs(posterior, np = np, pars = c("mu_base", "sd_base", "w[1]", "w[2]", "w[3]"))#,"w", "d"))
mcmc_pairs(posterior, np = np, pars = c("mu_base", "sd_base", "d[1]", "d[2]", "d[3]"))#,"w", "d"))
