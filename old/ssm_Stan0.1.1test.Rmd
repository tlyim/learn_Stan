...
simu_fit <- stan(#model_code = simu_SSM@model_code,
                 file = "simu_SSM0.1.1test.stan",  # Stan program
...
initf2 <- function(chain_id = 1) {
  list(mu_alpha01 = mu_alpha01, mu_alpha = mu_alpha, beta = beta,  
       ...
       rho_ST = rho_ST, rho = rho, sd_pi = sd_pi, rho_raw = rho_raw, alpha_ratio = alpha_ratio,     
...
file = "SSM0.1.1test.stan",  # Stan program
...
print(fit2, c("mu_u1", "mu_alpha", "beta", "rho_raw", "p",#"sd_pi",  #"rho_ST", 
...
pairs_stan(1, fit2, c("alpha_ratio", "beta", "rho_raw", "p"))#"season_raw"))#, 
...
plot(params.df$iter, params.df$alpha_ratio, pch=16, cex=0.8, #col=c_dark, 
     xlab="Iteration")#, ylim=c(0, 0.3))#, ylab="log(tau)", ylim=c(-6, 4))
plot(params.df$iter, log(params.df$rho_raw), pch=16, cex=0.8, #col=c_dark, 
      xlab="Iteration")#, ylab="log(tau)")#, ylim=c(-6, 4))
...
# alpha_ratio = 0.1, rho_raw = log(rho) = 0.693;
    # g = c(0.5, -0.8, 2.5)
    # w = c(-3.5, 7, 2)
    # d = c(0.05, 0.7, 0.25) 
