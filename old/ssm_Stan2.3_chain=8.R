...
#knitr::purl("ssm_Stan2.3.Rmd", output = "ssm_Stan2.3.R", documentation = 2)
...
J = 100#200 #00#50    # number of firms
N = 20*4#4#2 #4 #7 number of (yearly) observations of a firm
...
simu_fit <- stan(#model_code = simu_SSM@model_code,
                 file = "simu_SSM2.3.stan",  # Stan program
...
initf2 <- function(chain_id = 1) {
  list(alpha01 = alpha01, beta = beta, # alpha = alpha, 
       g = g, w = w,
       sd_y = sd_y, sd_base = sd_base, 
       sd_gamma = sd_gamma, sd_omega = sd_omega)
  }
