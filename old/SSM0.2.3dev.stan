...
parameters {
  real mu_u1;
  real<lower=0,upper=1> mu_alpha; //intercept coeff. (drift) of the AR(1) process of the unbiased figure y[n]
...
//mu_alpha ~ exponential(2);//normal(0, 1);//student_t(3, 0, 1);//
mu_alpha ~ normal(0.5, 0.5);
  beta ~ normal(0.5, 0.5);
