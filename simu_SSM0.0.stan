...
// Define seasaonality component
  for (j in 1:J) {
    for (s in 1:(Q-1)) {
      season_raw[j,s] = mu_season[s] + sd_season*normal_rng(0,1);
      }
...
//    alpha[j,1] = mu_alpha;   // y should be nonnegative for Rev and nonpositive for Costs
...
//      u[j,n] = alpha[j,n-1] + beta*u[j,n-1] + sd_y*normal_rng(0,1);// + sd_y*normal_rng(0,1); 
      u[j,n] = season_n[j,n] + mu_alpha + beta*u[j,n-1] + sd_y*normal_rng(0,1);// + sd_y*normal_rng(0,1); 
      }
