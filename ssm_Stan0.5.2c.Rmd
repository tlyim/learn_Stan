...
#knitr::purl("ssm_Stan0.5.2c.Rmd", output = "ssm_Stan0.5.2c.R", documentation = 2)
...
simu_fit <- stan(#model_code = simu_SSM@model_code,
                 file = "simu_SSM0.5.2c.stan",  # Stan program
...
#stan_rdump(names(SSM_data), file = "SSM_data_J54b.dat", envir = list2env(SSM_data)) 
...
metric = "diag_"#"dense_"#
#=================================
n_iter = 800#1000#1400#2000#2000#3000
n_refresh = max(2, n_iter/10)
n_warmup = 600#600#1000
#n_warmup = n_iter/2 
n_chains = 4#3
...
file = "SSM0.5.2c.stan",  # Stan program
...
#  fit = fit0_debug,   # to save compilation time if the debug model was run
  control = list(adapt_delta = 0.82,#0.89_800max,#0.88_2dvg,# 0.955,#0.95_1dvg,#0.82,#0.85,#8, #9
#                 adapt_engaged = 1,
 adapt_init_buffer = 75,#85,#75
 adapt_term_buffer = 50,#150,
 adapt_window = 25,#35,#10,#12,#15,
#                 stepsize = 0.05, #0.05, #0.01
                 metric = paste0(metric, "e"),
                 max_treedepth = 12#12#11
...
get_sampler_params(fit2)[[1]] %>% summary()
cat("\nseed:\n")
get_seed(fit2)
cat("\n")
...
mcmc_pairs(fit.ar, np = np, pars = c("mu_u1", "sd_y", "d[1]", "d[2]", "d[3]")) 
...
mcmc_pairs(fit.ar, np = np, pars = c("ab_mu[1]", "ab_mu[2]", "pgw_mu[1]", "pgw_mu[2]", "pgw_mu[3]"))
mcmc_pairs(fit.ar, np = np, pars = c("ab_mu[1]", "ab_mu[2]", "pgw_mu[1]", "p[2]", "p[3]"))
mcmc_pairs(fit.ar, np = np, pars = c("ab_mu[1]", "ab_mu[2]", "pgw_mu[2]", "g[2]"))
mcmc_pairs(fit.ar, np = np, pars = c("ab_mu[1]", "ab_mu[2]", "pgw_mu[3]", "w[2]", "w[3]"))
mcmc_pairs(fit.ar, np = np, pars = c("ab_mu[1]", "ab_mu[2]", "d[1]", "d[2]", "d[3]"))

mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[1]", "pgw_mu[2]", "pgw_mu[3]", "theta", "mu_season[1]"))
mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[1]", "pgw_mu[2]", "pgw_mu[3]", "mu_season[2]", "mu_season[3]"))
mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[1]", "pgw_mu[2]", "pgw_mu[3]", "d[1]", "d[2]", "d[3]"))

mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[1]", "p[2]", "p[3]", "theta", "mu_season[1]"))
mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[1]", "p[2]", "p[3]", "mu_season[2]", "mu_season[3]"))
mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[1]", "p[2]", "p[3]", "d[1]", "d[2]", "d[3]"))

mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[2]", "g[2]", "theta", "mu_season[1]"))
mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[2]", "g[2]", "mu_season[2]", "mu_season[3]"))
mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[2]", "g[2]", "d[1]", "d[2]", "d[3]"))

mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[3]", "w[2]", "w[3]", "theta", "mu_season[1]"))
mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[3]", "w[2]", "w[3]", "mu_season[2]", "mu_season[3]"))
mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[3]", "w[2]", "w[3]", "d[1]", "d[2]", "d[3]"))
...
mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[2]", "g[2]", "pgw_mu[1]", "p[2]", "p[3]"))
mcmc_pairs(fit.ar, np = np, pars = c("pgw_mu[2]", "g[2]", "pgw_mu[3]", "w[2]", "w[3]"))
...
get_divergent_iterations(fit2)
    ...
    # get_adaptation_info(fit2) # step size and full inverse mattrix
