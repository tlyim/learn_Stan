...
* Probability of state given the data is the probability of the state given the data up to that time step multiplied by the probability of future data
...
```{r, echo=TRUE, results='markdown', fig.height=3}
...
```{r}
calcofi = read.csv("calcofi.csv", stringsAsFactors = FALSE)
ggplot(calcofi, aes(year, m, group=species,col=species)) + geom_line() + ylab("CPUE") + xlab("Year")
```
...
* For example, if L_ochotensis time series was missing first hald of values
...
* Very sstraightforward to build these models in BUGS/JAGS
